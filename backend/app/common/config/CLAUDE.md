...
| Admin | `admin_password`, `admin_password_hash`, `cookie_secret` |
...
`admin_password` is a **bootstrap convenience**: on first start it is hashed
into `admin_password_hash` and the hash is persisted. Only the hash is stored
and only the hash is used afterwards.
