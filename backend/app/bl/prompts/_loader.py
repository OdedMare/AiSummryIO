"""Read prompt markdown from disk, resolve includes, and cache the result.

Prompts are product wording that gets tuned by reading it, so they live as
markdown beside the code rather than as Python string constants. The loader is
the only thing that knows they are files at all: callers ask for a name and
get a string, exactly as they used to get a module attribute.

Composition is an include line rather than `%s` interpolation, because the
shared fragments are shared *wording* and an include says where the text comes
from at the point it lands:
...
_SUFFIX = ".md"
_INCLUDE = re.compile(r"^[ \t]*<!--[ \t]*include:[ \t]*(?P<name>[^>]+?)[ \t]*-->[ \t]*$", re.M)

# Read once: `load` sits inside workflow execution and interview turns, both
# hot enough that re-reading per call would be waste for text that cannot
...
def load(name: str, reload: bool = False) -> str:
    ...
    `reload` re-reads from disk, which is the point of having these as files:
    wording can be iterated without restarting the process.
    """
    if reload:
...
def _compose(name: str, stack: List[str]) -> str:
    if name in stack:
        raise ValueError(
            "prompt include cycle: " + " -> ".join(stack + [name])
        )
    text = _read(name)
    return _INCLUDE.sub(
        lambda match: _compose(
            _normalized(match.group("name")), stack + [name]
        ),
        text,
    ).strip()
...
def _read(name: str) -> str:
    path = _path(name)
    if path is None:
        raise ValueError("prompt not found: " + name)
...
def _path(name: str) -> Optional[Path]:
    ...
    guard against a typo climbing out of the package, not a security boundary.
    """
