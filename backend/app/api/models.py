...
class FollowUpCreate(BaseModel):
    """A message in an existing conversation, optionally about a citation.

    ``citation_id`` is what the UI sends when the user asks a follow-up while
    a citation or evidence record is selected: "הצג לי את הרשומה הזו" then
    resolves deterministically to that exact source instead of being routed
    as a fresh search. ``referenced_citation_ids`` carries any further markers
    the question refers to. Both are optional, so every existing client keeps
    posting exactly the body it always did.
    """
    ...
    @classmethod
    def valid_citation_id(cls, value):
        if value is None:
            return None
        cleaned = value.strip()
        return cleaned[:_CITATION_ID_MAX_LENGTH] or None

    @field_validator("referenced_citation_ids")
    @classmethod
    def valid_referenced_citations(cls, values: List[str]) -> List[str]:
        cleaned = list(dict.fromkeys(
            value.strip()[:_CITATION_ID_MAX_LENGTH]
            for value in values if value.strip()
        ))
        if len(cleaned) > _MAX_REFERENCED_CITATIONS:
            raise ValueError("אפשר להפנות עד 10 ציטוטים")
        return cleaned
...
# A citation id is generated by the backend ("c1", "c2", …). Bounding it here
# keeps a hostile client from sending an unbounded string into a lookup.
_CITATION_ID_MAX_LENGTH = 64
_MAX_REFERENCED_CITATIONS = 10
